...
class CourseAdvanceSettingViewTest(CourseTestCase, MilestonesTestCaseMixin):
    ...
    @ddt.unpack
    def test_discussion_fields_available(self, is_pages_and_resources_enabled, is_legacy_discussion_setting_enabled):
        ...
        with override_waffle_flag(ENABLE_PAGES_AND_RESOURCES_MICROFRONTEND, is_pages_and_resources_enabled):
            with override_waffle_flag(OVERRIDE_DISCUSSION_LEGACY_SETTINGS_FLAG, is_legacy_discussion_setting_enabled):
                ...
                if (is_pages_and_resources_enabled and is_legacy_discussion_setting_enabled) or \
                    (not is_pages_and_resources_enabled and not is_legacy_discussion_setting_enabled):
                    self.assertTrue('allow_anonymous' in response)
                    self.assertTrue('allow_anonymous_to_peers' in response)
                    self.assertTrue('discussion_blackouts' in response)
                    self.assertTrue('discussion_topics' in response)
                elif is_pages_and_resources_enabled and not is_legacy_discussion_setting_enabled:
                    self.assertFalse('allow_anonymous' in response)
                    self.assertFalse('allow_anonymous_to_peers' in response)
                    self.assertFalse('discussion_blackouts' in response)
                    self.assertFalse('discussion_topics' in response)
